...
def plot_best_fit(weights):
    ...
    x = np.arange(-3.0, 3.0, 0.1) #返回从-3.0开始到0.1结束 步长为3.0的数列
...
def colic_test():
    ...
    :return:
    """
    f_train = open('./data/HorseTraining.txt', 'r')
    f_test = open('./data/HorseTest.txt', 'r')
    training_set = []
    training_labels = []
    ...
    for line in f_train.readlines():
        ...
        line_arr = [float(curr_line[i]) for i in range(22)]
        training_set.append(line_arr)
        training_labels.append(float(curr_line[22]))
    ...
    for line in f_test.readlines():
        ...
        if len(curr_line) == 1:
            continue    # 这里如果就一个空的元素，则跳过本次循环
        line_arr = [float(curr_line[i]) for i in range(22)]
        if int(classify_vector(np.array(line_arr), train_weights)) != int(curr_line[22]):
            error_count += 1
